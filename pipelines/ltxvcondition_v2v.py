...
# copied from diffusers, added V2V support 
...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if latents is not None:
            ...
            latents = self.scheduler.scale_noise(latents, timesteps, noise)
        else:
    ...
    def __call__(
        ...
        if v2v_strength is not None:
            timesteps, num_inference_steps = self.get_timesteps(num_inference_steps, timesteps, v2v_strength)
        ...
        latents, conditioning_mask, video_coords, extra_conditioning_num_latents = self.prepare_latents(
            ...
            dtype=torch.float32,
            latents=latents,
            timesteps=latent_timestep,
        )

        # print("prepare_latents", latents.shape)
